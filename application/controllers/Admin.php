	...
	function general_settings($para1="")
	{
		if ($this->admin_permission() == FALSE) {
			...
			elseif ($para1=="update_general_settings") {
				...
				$data15['value'] = $this->input->post('blur_member_profile_image');
				$this->db->where('type', 'blur_member_profile_image');
				$this->db->update('general_settings', $data15);

				$data16['value'] = $this->input->post('member_name_show_on_accept_interest');
				$this->db->where('type', 'member_name_show_on_accept_interest');
				$this->db->update('general_settings', $data16);
