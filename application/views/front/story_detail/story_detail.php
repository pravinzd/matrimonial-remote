...
foreach ($get_story as $value) {
    $check_interests = $this->Crud_model->get_type_name_by_id('member', $this->session->userdata('member_id'), 'interest');
    $check_interest = json_decode($check_interests, true);
    ...
    if ($member_name_show_on_accept_interest == 'yes') {
        if (!empty($this->session->userdata('member_id'))) {
            $name_show =  interest_status_check($value->posted_by, $check_interest);
        } else {
            $name_show = false;
        }
    } else {
    ...
    <div class="mb-4">
        ...
        <ul class="inline-links inline-links--style-2 mt-1">
            ...
            <li>
                ...
                ?>
                    <a class="c-base-1" href="<?= base_url() ?>home/member_profile/<?= $value->posted_by ?>">
                        <?php
                        if ($name_show) {
                            echo $this->Crud_model->get_type_name_by_id('member', $value->posted_by, 'first_name') . " " . $this->Crud_model->get_type_name_by_id('member', $value->posted_by, 'last_name');
                        } else {
                            echo 'ID' . '-' . $this->Crud_model->get_type_name_by_id('member', $value->posted_by, 'member_profile_id');
                        } ?>
                    </a>
                <?php
                }
    ...
    <section class="swiper-js-container background-image-holder swiper_custom" data-holder-type="fullscreen" data-holder-offset=".navbar">
        <div class="swiper-container swiper-container-horizontal swiper-container-fade" data-swiper-autoplay="true" data-swiper-effect="fade" data-swiper-items="1" data-swiper-space-between="0" data-swiper-sm-items="1" data-swiper-sm-space-between="0" data-swiper-xs-items="1" data-swiper-xs-space-between="0">
            <div class="swiper-wrapper" style="transition-duration: 0ms;">
                ...
                foreach ($images as $image) : ?>
                    <div class="swiper-slide <?php if ($i == 0) {
                                                ...
                                                } ?>" data-swiper-autoplay="5000" style="width: 1343px; opacity: 1; transform: translate3d(0px, 0px, 0px); transition-duration: 0ms;">
                        <div class="slice holder-item holder-item-light has-bg-cover bg-size-cover" style="background-image: url('<?= base_url() ?>uploads/happy_story_image/<?= $image['img'] ?>'); background-position: center bottom;">
                        </div>
                    </div>
                <?php
                    $i++;
            ...
            <div class="swiper-pagination swiper-pagination-clickable swiper-pagination-bullets">
                ...
                $j = 0;
                foreach ($images as $image) : ?>
                    <span class="swiper-pagination-bullet <?php if ($j == 0) {
    ...
    if ($video_exist) {
        ...
        foreach ($get_video as $video) { ?>
            <div class="post-media text-center pt-1 pb-3">
                ...
                <?php } else { ?>
                    <iframe controls="2" height="450" width="80%" src="<?php echo $video['video_src']; ?>" frameborder="0">
                    </iframe>
                <?php } ?>
            </div>
    <?php
