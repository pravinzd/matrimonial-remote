...
<div id="content-container">
	<div id="page-head">
		...
		<ol class="breadcrumb">
			...
			<li class="active"><a href="#"><?php echo translate('city') ?></a></li>
		</ol>
		<!--~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~-->
		<!--End breadcrumb-->
	</div>
	...
	<div id="page-content">
		...
		<div class="panel">
			...
			<div class="panel-heading">
				<h3 class="panel-title"><?php echo translate('cities') ?></h3>
			</div>
			<div class="panel-body">
				<div class="row">
					<div class="col-12 pull-right" style="padding-bottom: 10px">
						<!-- <a href="<?= base_url() ?>admin/city/add_city" class="btn btn-primary btn-sm btn-labeled fa fa-plus">Create New</a> -->
						<button data-target="#city_modal" data-toggle="modal" id="add_city" class="btn btn-primary btn-sm btn-labeled fa fa-plus"><?php echo translate('create_new') ?></button>
					</div>
				</div>
				<div class="row">
...
<!--===================================================-->
<div class="modal fade" id="city_modal" role="dialog" tabindex="-1" aria-labelledby="city_modal" aria-hidden="true">
	<div class="modal-dialog">
		<div class="modal-content">
			...
			<div class="modal-footer">
				...
				<button class="btn btn-primary btn-sm" id="save_city" value="0"><?php echo translate('save') ?></button>
			</div>
		</div>
	</div>
</div>
	...
	<div class="modal-dialog" style="width: 400px;">
		<div class="modal-content">
			...
			<div class="modal-body">
				...
				<div class="text-end">
					<button data-dismiss="modal" class="btn btn-default btn-sm" type="button" id="modal_close"><?php echo translate('close') ?></button>
					<button class="btn btn-danger btn-sm" id="delete_city" value=""><?php echo translate('delete') ?></button>
				</div>
			</div>
...
<script>
	$(document).ready(function() {
		$('#city_table').DataTable({
			...
			"serverSide": true,
			"ajax": {
				"url": "<?php echo base_url('admin/city/list_data') ?>",
				"dataType": "json",
				"type": "POST",
				"data": {
...
<script>
	...
	function edit_city(id) {
		$("#modal_title").html("Edit City");
		$("#modal_body").html("<div class='text-center'><i class='fa fa-refresh fa-5x fa-spin'></i></div>");
		$("#save_city").val(2);
		$('#validation_info').html("");
		$.ajax({
			url: "<?= base_url() ?>admin/city/edit_city/" + id,
			success: function(response) {
				$("#modal_body").html(response);
			},
			fail: function(error) {
	...
	$("#save_city").click(function() {
		var check = $("#save_city").val();
		var form = $("#city_form");
		if (check == 1) {
			var page_url = "<?= base_url() ?>admin/city/do_add"
		}
		if (check == 2) {
			var page_url = "<?= base_url() ?>admin/city/update"
		}
		$.ajax({
			type: "POST",
			url: page_url,
	...
	$("#delete_city").click(function() {
		$.ajax({
			url: "<?= base_url() ?>admin/city/delete/" + $("#delete_city").val(),
			success: function(response) {
				window.location.href = "<?= base_url() ?>admin/city";
			},
			fail: function(error) {
...
<script>
	function load_state(id) {
		$.ajax({
			url: "<?= base_url() ?>admin/load_state_with_country_id/" + id,
			success: function(response) {
